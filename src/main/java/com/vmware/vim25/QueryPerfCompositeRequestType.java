package com.vmware.vim25;

import com.vmware.vim25.ManagedObjectReference;
import com.vmware.vim25.PerfQuerySpec;
import com.vmware.vim25.QueryPerfCompositeRequestType;
import javax.xml.bind.annotation.XmlAccessType;
import javax.xml.bind.annotation.XmlAccessorType;
import javax.xml.bind.annotation.XmlElement;
import javax.xml.bind.annotation.XmlType;

@XmlAccessorType(XmlAccessType.FIELD)
@XmlType(name = "QueryPerfCompositeRequestType", propOrder = {"_this", "querySpec"})
public class QueryPerfCompositeRequestType {
  @XmlElement(required = true)
  protected ManagedObjectReference _this;
  
  @XmlElement(required = true)
  protected PerfQuerySpec querySpec;
  
  public ManagedObjectReference getThis() {
    return this._this;
  }
  
  public void setThis(ManagedObjectReference paramManagedObjectReference) {
    this._this = paramManagedObjectReference;
  }
  
  public PerfQuerySpec getQuerySpec() {
    return this.querySpec;
  }
  
  public void setQuerySpec(PerfQuerySpec paramPerfQuerySpec) {
    this.querySpec = paramPerfQuerySpec;
  }
}
